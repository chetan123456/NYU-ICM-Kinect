...
void draw() {
  ...
  //Collision detection for blob and oscillating objects-----------------
  for (int l = 0; l < blobs.size(); l++){
   for(int m = 0; m < oscillators.length; m++){
     if (blobs.get(l).checkCollision(oscillators[m].x, oscillators[m].y)) {
       oscillators[m].oscillatorStop = blobs.get(l).checkCollision(oscillators[m].x, oscillators[m].y);
     }
     println(oscillators[m].oscillatorStop);
   }
  }
...
void keyPressed() {
  ...
  if (key == CODED) {
    if (keyCode == UP) {
      ...
      for(int q = 0; q < oscillators.length; q++){
       oscillators[q].oscillatorStop = false;
      }
    }
  }
}
